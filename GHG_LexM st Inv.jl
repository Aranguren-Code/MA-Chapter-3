# QUESTIONS - DOES NOT RUN - OUT OF BOUNDS SOLUTION
# Loading the necessary packages
#-------------------------------------------------------------------------------
# OPTIMIZING TO MINIMIZE EMISSIONS S.T. INVESTMENT
#-------------------------------------------------------------------------------
...
#CPX_PARAM_BENDERSSTRATEGY = 3))
#set_parameter(HS_P3, "CPXPARAM_Benders_Strategy", 3) #works in work-station

#HS_P3 =  Model(with_optimizer(CPLEX.Optimizer, CPX_PARAM_BENDERSSTRATEGY = 3));
...
@constraint(HS_P3, 
  ...
  sum(prob[s]*(sum((HarvCost[i] + TransCost1[i, j] + Ash[i]) * FLOWX[i, j, s] for i=1:P, j=1:D) +
	...
	sum((HarvCost[i] + TransCost3[i, j] + Ash[i]) * FLOWZ[i, j, s] for i=1:P, j=1:C)) for s=1:S) == Best_Inv);

#----------------------------------------------------------------
#                 Min. Emissions Objective Function
# ---------------------------------------------------------------
@objective(HS_P3, Min, 
