...
#-------------------------------------------------------------------------------
# OPTIMIZING TO MIN INV S.T. EMISSIONS
#-------------------------------------------------------------------------------
...
@constraint(HS_P3,
  ...
  sum((EmissionsCost3[i,j]) * FLOWZ[i, j, s] for i=1:P, j=1:C)) for s=1:S) == Best_Em)  ;
...
@objective(HS_P3, Min, 
	...
	sum((HarvCost[i] + TransCost3[i, j]+ Ash[i]) * FLOWZ[i, j, s] for i=1:P, j=1:C)) for s=1:S));
